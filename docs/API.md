...
网关服务（默认 `http://localhost:8000`）对外暴露的全部 **30 个 HTTP 接口**规范。

## 全局约定

- 内容类型：`application/json`（二进制上传 / 下载与 SSE 接口除外）；
- 时间戳：Unix 秒级整数。

### 通用响应信封

除特殊说明外，所有接口的响应都套用此结构：
...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `requestId` | 是 | 请求链路追踪 ID，所有请求必填 |
| `sessionId` | 视接口 | 登录凭证；健康检测、唯一性校验、注册、密码登录、验证码获取 / 登录接口不需要 |
| `chatSessionId` | 视接口 | 与 AI 模型的聊天会话 ID（注意与 `sessionId` 区分） |
...
| `codeId` | 验证码 ID，与 `verifyCode` 配对用于验证码登录或注册 |
| `fileId` | 上传文件 ID |
| `connectionId` / `dbConnectId` | 数据库连接 ID（默认连接 `excel_default` 专用于智能 Excel 场景） |
...
返回结果：无
注意：verifyCode 与 codeId 通过 [U05] 获取，验证码校验通过才能注册成功
...
POST /api/user/code
请求体：{ "requestId": str, "email": str }
返回结果：{ "codeId": str }   // codeId + 邮箱收到的 verifyCode 配对用于 [U06]
...
dbConnectionInfo：数据库连接信息 JSON，database 类型会话必填，excel 类型不填
...
| U09 | POST | `/api/user/info` | 获取用户信息（query 参数） |
