...
// 分析提示词中出现的真实占位符样例
constexpr const char* kAnalyzePlaceholder = "{DataBase}";
...
{
    // 正常模板构建成功
    PromptTemplate normal_template(BuildTestTemplate());
    (void)normal_template;

    // 空模板构建失败, 抛出提示词模板错误码异常
    try
    {
        PromptTemplate empty_template("");
        FAIL() << "空模板构建应该抛出异常";
    }
...
// 占位符提取测试 : 验证合法标识符被提取, JSON 花括号等内容不被误判
TEST(PromptTemplateTest, ExtractPlaceholders)
{
    PromptTemplate prompt_template(BuildTestTemplate());
