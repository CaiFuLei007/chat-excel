...
任务 : 完成AI聊天子服务操作数据的封装 , 包括两个部分 : 对 odb 生成的代码进行封装 , 对 redis 操作进行封装

## 1. 用户聊天会话表

1. odb 对 ChatSessionData 类编译生成的代码在 /chat-excel/data/odb 目录下 
2. 定义 ChatSessionData 类 , 包含在 chat_excel::ai_service 命名空间中
3. 头文件 chat_session_data.h , 源文件 chat_session_data.cpp , 放在 /chat-excel/data 路径下
4. 类中包含两个句柄 , 分别是 MySQL 操作句柄 和 Redis 操作句柄
5. Mysql 句柄使用 <cpp-toolkit/odb.h> 中封装的代码实现 , redis 句柄使用 <cpp-toolkit/redis.h> 中封装的代码实现
6. ChatSessionData 类涉及到的 MySQL 操作如下 : 
	1. 保存或更新会话信息到数据库
	2. 通过会话 ID 获取指定会话信息
	3. 通过会话 ID 删除指定会话信息
    4. 通过 user_id 获取用户所有会话信息
7. ChatSessionData 类涉及到的 Redis 操作如下 : 
	1. 保存或更新会话数据到缓存
    2. 通过会话 ID 获取指定会话信息
    3. 通过会话 ID 删除指定会话信息
8. ChatSessionData 缓存策略 : Cache-Aside 旁路策略 ; 过期时间为 3天 
	1. 写策略 : 删除缓存中的数据 , 修改 MySQL 数据库 , 删除缓存中的数据
	2. 读策略 : 先从缓存中进行读取 , 如果没有再到 MySQL 中进行读取 , 再将数据添加到缓存中
	3. 删除或注销 : 依赖 TTL 自然过期 
	4. 缓存类型 : hash 类型 , key 是 chat_session_data , 
        1. field : chat_session:{session_id}
		2. value : 一个 JSON 字符串 包含 : 会话信息 : 会话 ID , 标题 , 创建时间 , 最近一次消息时间 , 总条数 , 模型名称 , 文件 ID , 会话类型 , 连接信息
	5. ChatSessionData 只实现对数据库和 Redis 的 增删查改操作 , 业务逻辑上层实现 , 不考虑什么时候加入删除缓存 , 只实现对应的接口

## 3. 注意事项

1. 如果需要定义通用结构体 , 将结构体放在 chat-excel/svc_ai_service 下的 common.h 中
...
5. 使用 gtest 测试框架对上面的两个类进行测试
	1. 测试 ChatSessionData 类的所有接口，包括 MySQL 操作和 Redis 操作。测试代码放在: chat-excel/test/svc_ai_service/chat_session_data 目录下
