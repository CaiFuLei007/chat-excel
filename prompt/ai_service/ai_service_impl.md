...
**任务** : 完成 AI 子服务的所有 RPC 接口实现

## 1. RPC 接口

1. RPC 接口的定义在 chat-excel/proto/ai_service.proto 
2. 在 chat-excel/proto/ 中添加 CMakeLists.txt 文件用于编译 proto 文件, 将生成的头文件和源文件存放到 chat-excel/proto/proto_code 中


## 2. AiServiceImpl

1. 定义 AiServiceImpl 类 , 包含在 chat_excel::ai_service 命名空间中 , 负责实现 AI相关的 RPC 接口
2. 头文件 ai_service_impl.h , 源文件 ai_service_impl.cpp , 放在 chat-excel/svc_ai_service 路径下
3. 成员 : 包含一个 AiBussiness 智能指针 , 用于调用业务逻辑处理接口 , AiBusiness 实例在外部构建 , 构造函数存入智能指针
4. 定义所有 RPC 接口 , 步骤如下
